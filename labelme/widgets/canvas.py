...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # add by ls
        self.pixmap_mask = QtGui.QPixmap()
    ...
    def paintEvent(self, event):
        ...
        # add by ls 
        pMap = self.pixmap_mask
        # pMap=QtGui.QPixmap(r'C:\Users\fcgy01\Desktop\ww3_mask\augsburg_000000_000000_rightImg8bit_prediction.png')
        temp = QtGui.QPixmap(pMap.size())
        temp.fill(QtCore.Qt.transparent)
        p1=QtGui.QPainter(temp)
        p1.setCompositionMode(QtGui.QPainter.CompositionMode_Source)
        p1.drawPixmap(0, 0, pMap)
        p1.setCompositionMode(QtGui.QPainter.CompositionMode_DestinationIn)
        p1.fillRect(temp.rect(), QtGui.QColor(0, 0, 0, 100))#根据QColor中第四个参数设置透明度，0～255
        p1.end()
        pMap = temp
        # self.pixmap = pMap
    ...
    def loadPixmap(self, pixmap,pixmap_mask, clear_shapes=True):
        self.pixmap = pixmap
        # add by ls
        self.pixmap_mask = pixmap_mask
        if clear_shapes:
            self.shapes = []
        self.update()
